...
ui <- fluidPage(
  navbarPage("IMCP: An Interactive Map of Climate and Phenology",
    id = "nav",
    tabPanel(
      "Mean Annual Temperature vs Start of Season",
      # div(class="outer",
      selectInput("layer", "Layer",
        choices = c("MAT Anomaly", "SOS Anomaly", "Mean MAT", "Mean SOS", "MAT Velocity", "SOS Velocity", "Projection", "Mismatch")
      ),
      sliderInput("year", "Year", min = 1981, max = 2014, value = 1, ticks = T),
      # If not using custom CSS, set height of leafletOutput to a number instead of percent
      leafletOutput("raster_map", height = 300, width = 650),
      absolutePanel(
        id = "controls", class = "panel panel-default", fixed = TRUE, draggable = TRUE, top = 120, left = "auto", right = 360, bottom = "auto", width = 300, height = "auto",
        h4("Temporal patterns"),
        plotOutput("lineplot_TMP", height = 200),
        plotOutput("lineplot_SOS", height = 200)
      ),
      absolutePanel(
        id = "controls", class = "panel panel-default", fixed = TRUE, draggable = TRUE, top = 120, left = "auto", right = 60, bottom = "auto", width = 300, height = "auto",
        h4("Spatial patterns"),
...
server <- function(input, output) {
  ...
  observe({
    if (input$layer == "MAT Anomaly") {
      ...
      leafletProxy("raster_map") %>%
        ...
        addLegend(
          pal = pal_tmpano, values = seq(-2, 2, by = 1),
    ...
    if (input$layer == "SOS Anomaly") {
      ...
      leafletProxy("raster_map") %>%
        ...
        addLegend(
          pal = pal_sosano, values = seq(-15, 15, by = 5),
    ...
    if (input$layer == "Mean MAT") {
      leafletProxy("raster_map") %>%
        ...
        addLegend(
          pal = pal_tmp, values = values(mnTMP),
    ...
    if (input$layer == "Mean SOS") {
      leafletProxy("raster_map") %>%
        ...
        addLegend(
          pal = pal_sos, values = values(mnSOS),
    ...
    if (input$layer == "SOS Velocity") {
      leafletProxy("raster_map") %>%
        ...
        addLegend(
          pal = qpal_Vp, values = values(mask(Vp, Vc)),
    ...
    if (input$layer == "Projection") {
      leafletProxy("raster_map") %>%
        ...
        addLegend(
          pal = qpal_projection, values = values(projection),
    ...
    if (input$layer == "Mismatch") {
      leafletProxy("raster_map") %>%
        ...
        addLegend(
          pal = qpal_mismatch, values = values(mismatch),
  ...
  observeEvent(input$raster_map_click, {
    ...
    tmp_neighbours <- matrix(
      c(
        extract(mnTMP, sp_top_left), extract(mnTMP, sp_top), extract(mnTMP, sp_top_right),
    ...
    sos_neighbours <- matrix(
      c(
        extract(mnSOS, sp_top_left), extract(mnSOS, sp_top), extract(mnSOS, sp_top_right),
